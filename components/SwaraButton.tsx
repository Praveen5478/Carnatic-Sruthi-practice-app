...
export const SwaraButton: React.FC<SwaraButtonProps> = ({ swara, isActive, onClick }) => {
  return (
    <button
      ...
      className={`
        relative w-full h-12 md:h-14 flex flex-col items-center justify-center rounded-xl
    ...
    >
      <span className={`text-base md:text-xl font-bold ${isActive ? 'text-white' : 'text-slate-200'}`}>
        {swara.label}
      </span>
      <span className={`text-[9px] uppercase tracking-wider font-semibold ${isActive ? 'text-indigo-200' : 'text-slate-500'}`}>
        {swara.shortLabel}
      </span>
    </button>
  );
